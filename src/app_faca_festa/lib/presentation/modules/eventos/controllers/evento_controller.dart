...
class EventoController extends GetxController {
  ...
  Future<void> selecionarEvento(Evento evento) async {
    ...
    try {
      _protegerEventoAtualAte = DateTime.now().add(const Duration(seconds: 12));
      await _inicializarEvento(evento);
    } finally {
  ...
  Future<void> _aplicarListaDoUsuario(List<Evento> eventos) async {
    ...
    if (idAtual != null &&
        eventos.any((evento) => evento.idEvento == idAtual)) {
      return;
    }
  ...
  Future<String?> enviarCapaEvento(List<int> bytes) async {
    ...
    try {
      final urlComVersao = await _repository.enviarCapa(
        idEvento: id,
        bytes: bytes,
      );
  ...
  Future<bool> removerCapaEvento() async {
    ...
    try {
      try {
        await _repository.removerArquivoCapa(id);
      } catch (e) {
